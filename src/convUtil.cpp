...
void _convBox(float *I, float *O, int h, int w, int d, int r, int s) {
  ...
  while(d-- > 0) {
    ...
    k++; if(k==s) { k=0; _convBoxY(T,O,h,r,s); O+=h/s; }
    for( i=1; i<w0; i++ ) {
...
void _conv11Y(float *I, float *O, int h, int side, int s) {
...
void _conv11(float *I, float *O, int h, int w, int d, int side, int s) {
  ...
  for( int d0=0; d0<d; d0++ ) for( i=s/2; i<w; i+=s ) {
    ...
    _conv11Y(T,O,h,side,s); O+=h/s;
  }
  alFree(T);
}

// convolve one column of I by a 2rx1 triangle filter
void _convTriY(float *I, float *O, int h, int r, int s) {
...
void _convTri(float *I, float *O, int h, int w, int d, int r, int s) {
  ...
  while(d-- > 0) {
    ...
    // prepare and convolve each column in turn
    k++; if(k==s) { k=0; _convTriY(U,O,h,r-1,s); O+=h/s; }
    for( i=1; i<w0; i++ ) {
...
void _convTri1Y(float *I, float *O, int h, float p, int s) {
...
void _convTri1(float *I, float *O, int h, int w, int d, float p, int s) {
  ...
  for( int d0=0; d0<d; d0++ ) for( i=s/2; i<w; i+=s ) {
    ...
    _convTri1Y(T,O,h,p,s); O+=h/s;
  }
  alFree(T);
}

// convolve one column of I by a 2rx1 max filter
void _convMaxY(float *I, float *O, float *T, int h, int r) {
...
void _convMax(float *I, float *O, int h, int w, int d, int r) {
...
void convTri(cv::Mat& input, cv::Mat& output, float r, int s) {
  ...
  if (r > 0 && r <= 1 && s <= 2) {
    convConst("convTri1", input, output, 12 / r / (r + 2) - 2, s);
  } else {
    convConst("convTri", input, output, r, s);
  }
}
