...
void gradient(CellArray& I, CellArray& Gx, CellArray& Gy) {
  ...
  if (h < 2 || w < 2) {
    wrError("I must be at least 2x2.");
  }
...
void gradientMag(CellArray& I, CellArray& M, CellArray& O, int channel, float normRad, float normConst, bool full) {
  ...
  gradMag(input, (float*)M.data, (float*)O.data, h, w, d, full);
...
void gradientHist(CellArray& M, CellArray &O, CellArray& H, int binSize, int nOrients, int softBin, int useHog, float clipHog, bool full) {
  ...
  if (useHog == 0) {
    gradHist((float*)M.data, (float*)O.data, (float*)H.data, h, w, binSize, nOrients, softBin, full);
  } else if (useHog == 1) {
    hog((float*)M.data, (float*)O.data, (float*)H.data, h, w, binSize, nOrients, softBin, full, clipHog);
  } else {
    fhog((float*)M.data, (float*)O.data, (float*)H.data, h, w, binSize, nOrients, softBin, clipHog);
  }
}
